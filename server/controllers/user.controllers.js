//contains core logic for user registration, login, logout, and admin controls
...
const validateStringField = (field, value) => {
  if (typeof value == "string" && value.trim() === "") {
    throw new ErrorHandler(`${field} is required`, 400); //for custom error message
  }
};
...
const validateDateField = (dob) => {
  const [day, month, year] = dob.split("/").map(Number); // Split the date string into day, month, and year and map it.
  const date = new Date(year, month - 1, day); //0 to 11 for month
